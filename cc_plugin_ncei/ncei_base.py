...
class BaseNCEICheck(BaseNCCheck):
    ...
    def _check_min_max_range(self, var, test_ctx):
        ...
        if 'valid_range' in var.ncattrs():
            test_ctx.assert_true(var.valid_range.dtype == var.dtype and
                                 ...
                                 "valid_range must be a two element vector of min followed by max with the same data type as {}".format(var)
                                 )



        else:
            test_ctx.assert_true(getattr(var, 'valid_min') is not None,
                                'valid_min attribute should exist and not be empty or valid_range should be defined')
            test_ctx.assert_true(getattr(var, 'valid_max') is not None,
                                'valid_max attribute should exist and not be empty')
        return test_ctx
    ...
    def check_height(self, dataset):
        ...
        self._check_min_max_range(var, recommended_ctx)
